...
// двусв€зный упор€доченный список
//
//

// структура двусв€зного списка
template <class T>
struct list {
	list* next, *last;
	T data;
...
// класс упор€доченного списка с итератором
template <class temp>
class ord_list
{
...
public:
	...
	// добавление элемента в соответсвующем пор€дке
	void add(temp a) {
		list<temp>* cur = new list<temp>;
		cur->data = a;
		if (h == NULL) {
